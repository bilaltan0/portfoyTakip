        ...
        {__DEV__ && (
          <>
            ...
            <View style={styles.testSection}>
              <Text style={styles.testTitle}>🧪 Phase 1 - Data Layer</Text>
            ...
            <View style={[styles.testSection, { borderColor: '#10B981' }]}>
              <Text style={styles.testTitle}>🧪 Phase 2 - Calculation Engine</Text>
              <Text style={styles.testText}>
              ...
              <TouchableOpacity 
                style={[styles.testButton, { backgroundColor: '#8B5CF6', marginBottom: 8 }]}
                onPress={async () => {
                  try {
                    // Önce temizle
                    const { clearAllSubCategoryData } = await import('../utils/subCategoryStorage');
                    await clearAllSubCategoryData();
                    
                    // Test kategorilerini oluştur
                    const testCategories = [
                      { name: 'Layer 1', parentCategory: 'Kripto', icon: '🔷', color: '#3B82F6', targetPercentage: 40 },
                      { name: 'DeFi', parentCategory: 'Kripto', icon: '🏦', color: '#8B5CF6', targetPercentage: 30 },
                      { name: 'Stable', parentCategory: 'Kripto', icon: '💵', color: '#10B981', targetPercentage: 20 },
                      { name: 'Teknoloji', parentCategory: 'Borsa', icon: '💻', color: '#06B6D4', targetPercentage: 60 },
                      { name: 'Havacılık', parentCategory: 'Borsa', icon: '✈️', color: '#F59E0B', targetPercentage: 40 },
                      { name: 'Fiziksel', parentCategory: 'Altın', icon: '🏆', color: '#FFD700', targetPercentage: 100 },
                    ];
                    ...
                    // Varlıkları ata
                    const { assignAssetToSubCategory: assignAsset } = await import('../utils/subCategoryStorage');
                    
                    // Kripto varlıkları
                    const cryptoAssets = assetsForPricing.filter(a => a.category === 'Kripto');
                    for (const asset of cryptoAssets) {
                      if (asset.name.includes('Ethereum') || asset.name.includes('Avalanche')) {
                        // Layer 1 kategorisine ata
                        const layer1 = subCategories.find(sc => sc.name === 'Layer 1');
                        if (layer1) await assignAsset(asset.name, layer1.id);
                      } else if (asset.name.includes('Mina')) {
                        // DeFi kategorisine ata (örnek olsun)
                        const defi = subCategories.find(sc => sc.name === 'DeFi');
                        if (defi) await assignAsset(asset.name, defi.id);
                      }
                    }
                    
                    // Borsa varlıkları
                    const stockAssets = assetsForPricing.filter(a => a.category === 'Borsa');
                    for (const asset of stockAssets) {
                      if (asset.name.includes('Hava')) {
                        const aviation = subCategories.find(sc => sc.name === 'Havacılık');
                        if (aviation) await assignAsset(asset.name, aviation.id);
                      }
                    }
                    
                    // Altın varlıkları
                    const goldAssets = assetsForPricing.filter(a => a.category === 'Altın');
                    for (const asset of goldAssets) {
                      const physical = subCategories.find(sc => sc.name === 'Fiziksel');
                      if (physical) await assignAsset(asset.name, physical.id);
                    }
                    ...
                    Alert.alert(
                      '✅ Test Data Hazır!',
                      `${testCategories.length} kategori oluşturuldu\n` +
                      `Varlıklar kategorilere atandı\n\n` +
                      `Şimdi hesaplama testlerini çalıştırabilirsiniz!`
                    );
                  } catch (error) {
                    Alert.alert('❌ Hata', error.message);
              ...
              >
                <Text style={styles.testButtonText}>🎯 Setup Test Data</Text>
              </TouchableOpacity>
              
              <View style={{ flexDirection: 'row', gap: 8, marginTop: 8 }}>
                <TouchableOpacity 
                  style={[styles.testButton, { flex: 1, backgroundColor: '#10B981' }]}
                  onPress={async () => {
                    try {
                      const { 
                        calculateAllSubCategoryAllocations,
                        getPortfolioSubCategorySummary 
                      } = await import('../utils/subCategoryCalculations');
                      
                      // Hesapla
                      const allocations = calculateAllSubCategoryAllocations(
                        subCategories,
                        assetsForPricing.map(a => ({
                          assetName: a.name,
                          mainCategory: a.category,
                          quantity: a.quantity,
                      ...
                      const summary = getPortfolioSubCategorySummary(allocations);
                      
                      Alert.alert(
                        '📊 Allocation Sonuçları',
                        `Toplam Alt Kategori: ${summary.total}\n` +
                        `✅ Hedefe Uygun: ${summary.onTrack}\n` +
                        `⚠️ Fazla Ağırlıklı: ${summary.overweight}\n` +
                        `⚠️ Az Ağırlıklı: ${summary.underweight}\n` +
                        `💤 Boş: ${summary.empty}\n\n` +
                        `🏥 Sağlık Skoru: %${summary.healthScore}`
                      );
                      
                      console.log('📊 Full Allocations:', JSON.stringify(allocations, null, 2));
                    } catch (error) {
                      Alert.alert('❌ Hata', error.message);
                    }
                  }}
                >
                  <Text style={styles.testButtonText}>📊 Allocations</Text>
                </TouchableOpacity>
                
                <TouchableOpacity 
                  style={[styles.testButton, { flex: 1, backgroundColor: '#F59E0B' }]}
                  onPress={async () => {
                    try {
                      const { 
                        calculateAllSubCategoryAllocations,
                      ...
                      const suggestions = getRebalancingSuggestions(allocations, totalPortfolioValue);
                      
                      if (suggestions.length === 0) {
                        Alert.alert('✅ Mükemmel!', 'Portföyünüz dengelendi, rebalancing gerekmez.');
                      } else {
                        const suggestionText = suggestions.slice(0, 3).map(s => 
                          `${s.icon} ${s.subCategory}\n` +
                          `${s.action === 'buy' ? '🔼 AL' : '🔽 SAT'}: ₺${Math.round(s.suggestedAmount).toLocaleString('tr-TR')}\n` +
                          `Hedef: %${s.targetPercentage} | Şu an: %${s.currentPercentage}\n` +
                          `${s.reason}`
                        ).join('\n\n');
                        
                        Alert.alert(
                          '⚖️ Rebalancing Önerileri',
                          `${suggestions.length} öneri bulundu:\n\n${suggestionText}`,
                          [{ text: 'Tamam' }]
                        );
                        
                        console.log('⚖️ Full Suggestions:', JSON.stringify(suggestions, null, 2));
                      }
                    } catch (error) {
                      Alert.alert('❌ Hata', error.message);
                    }
                ...
                >
                  <Text style={styles.testButtonText}>⚖️ Rebalance</Text>
                </TouchableOpacity>
              </View>
              
              <TouchableOpacity 
                style={[styles.testButton, { backgroundColor: '#6366F1', marginTop: 8 }]}
                onPress={async () => {
                  try {
                    const { calculateCategoryBreakdown } = await import('../utils/subCategoryCalculations');
                    
                    const breakdown = calculateCategoryBreakdown(
                      'Kripto', // Test için Kripto kategorisi
                      subCategories,
                      assetsForPricing.map(a => ({
                        assetName: a.name,
                        mainCategory: a.category,
                    ...
                    const categoryText = breakdown.subCategories.map(sc => 
                      `${sc.icon} ${sc.name}\n` +
                      `Hedef: %${sc.targetPercentage} | Gerçek: %${sc.actualPercentage.toFixed(1)}\n` +
                      `Değer: ₺${Math.round(sc.actualValue).toLocaleString('tr-TR')} | ${sc.status.toUpperCase()}`
                    ).join('\n\n');
                    
                    Alert.alert(
                      `📊 ${breakdown.mainCategory} Breakdown`,
                      `Toplam: ₺${Math.round(breakdown.total).toLocaleString('tr-TR')}\n` +
                      `Alt Kategori Sayısı: ${breakdown.subCategories.length}\n\n` +
                      (breakdown.subCategories.length > 0 ? categoryText : 'Henüz alt kategori yok') +
                      `\n\n📦 Kategorisiz: ₺${Math.round(breakdown.uncategorized.total).toLocaleString('tr-TR')} (%${breakdown.uncategorized.percentage.toFixed(1)})`
                    );
              ...
              >
                <Text style={styles.testButtonText}>� Kripto Breakdown</Text>
              </TouchableOpacity>
            </View>
          </>
        )}
